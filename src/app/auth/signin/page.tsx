...
const RESEND_COOLDOWN = 90; // 90 seconds
...
export default function SignInPage() {
  ...
  const handleDigitChange = (i: number, value: string) => {
    ...
    if (clean.length > 1) {
      ...
      if (pasted.length === 6) {
        ...
        setTimeout(() => focusCell(Math.min(pasted.length, 5)), 0);
      }
      return;
    }
  ...
  const sendOtp = useCallback(async () => {
    const res = await fetch("/api/auth/otp/send", {
      ...
      body: JSON.stringify({ email: email.trim(), password, type: "signin" }),
    });
  ...
  // Step 2: password → verify password + send OTP
  ...
  const handleOtpSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/otp/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim(), code: code.trim(), type: "signin" }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const subtitles = {
    email: "Sign in to continue your learning quest",
    password: `Signing in as ${email}`,
    otp: `We sent a code to ${email}`,
  };

  return (
    <div className="auth-page px-4 py-8 sm:py-12">
      ...
      <div className="w-full max-w-[384px] relative">
        <div className="quest-frame rounded-2xl bg-bg-card/70 backdrop-blur-sm border border-border-subtle p-5 sm:p-8 animate-fade-in-scale">
          <div className="text-center mb-6">
            ...
            <h1 className="text-lg font-bold tracking-tight">{titles[step]}</h1>
            <p className="text-text-muted text-[13px] mt-1.5 leading-relaxed">{subtitles[step]}</p>
          </div>
          ...
          {step === "email" && (
            <form onSubmit={handleEmailSubmit} className="space-y-4 animate-fade-up">
              ...
              <button type="submit" className="btn-primary btn-tactile w-full mt-2">
                Continue
              </button>
            </form>
          )}
          ...
          {step === "password" && (
            <form onSubmit={handlePasswordSubmit} className="space-y-4 animate-fade-up">
              {error && <ErrorBanner message={error} />}
              <div>
              ...
              <button type="submit" disabled={loading} className="btn-primary btn-tactile w-full mt-2">
                {loading ? <InlineLoader text="Verifying..." /> : "Continue"}
              </button>
          ...
          {step === "otp" && (
            <form onSubmit={handleOtpSubmit} className="space-y-5 animate-fade-up">
              ...
              <button
                type="submit"
                disabled={loading || !otpComplete || otpTimer === 0}
              ...
              >
                {loading ? (
                  ...
                  <span className="flex items-center justify-center gap-2"><SproutIcon className="w-4 h-4" />Resume my quest</span>
                ) : "Sign In"}
              </button>
              <button type="button" onClick={() => { setStep("password"); setDigits(["", "", "", "", "", ""]); setError(""); }} className="btn-ghost w-full">
                &larr; Back
              </button>
            </form>
          )}
          ...
          <div className="text-center mt-6">
            <Link href="/auth/forgot-password" className="text-[12px] text-text-dim hover:text-text-muted transition-colors">
              Forgot password?
            </Link>
          </div>
